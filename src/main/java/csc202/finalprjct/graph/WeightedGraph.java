package csc202.finalprjct.graph;
...
// Create a ShortestPathTree
    ...
    public class ShortestPathTree extends Tree {
         ...
         * Print paths from all vertices to the source
         */
        public String printAllPathsToString() {
            StringBuilder result = new StringBuilder("All shortest paths from " +
                    vertices.get(getRoot()) + " are:");
            for (int i = 0; i < cost.length; i++) {
                result.append(printPathToString(i)); // Print a path from i to the source
                result.append("(cost: ").append(cost[i]).append(")"); // Path cost
            }
            return result.toString();
        }
    }

//    public List<V> getOptimalPath() {
//
//    }
}